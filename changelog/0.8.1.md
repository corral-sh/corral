...
**Upgrading:** `corral upgrade` (or `brew update && brew upgrade corral-sh/tap/corral`). No template change: existing boxes are not drifted and need no rebuild. **Scripts:** `golden prune`, `rebuild`, `delete` and `uninstall` run without a terminal now fail unless given `--yes` (they used to exit 0 having done nothing, #33).
